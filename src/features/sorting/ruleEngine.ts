...
export function evaluateMaterialFallback(materialCode: BroadMaterialCode): RuleEngineResult {
  ...
  const preparationActions = guidance.steps.map((text, index) => ({
    text,
    textVi: guidance.stepsVi[index] ?? text,
    components: [],
  }))
  ...
  return {
    ...
    mainInstructionVi: guidance.stepsVi.at(-1) ?? guidance.whyVi,
    detailedInstructionVi: guidance.whyVi,
    whyCategoryVi: guidance.whyVi,
    preparationSteps: guidance.steps,
    preparationStepsVi: guidance.stepsVi,
    preparationActions,
    componentActions: [],
